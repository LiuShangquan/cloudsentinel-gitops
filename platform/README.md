...
本次学生集群由平台管理员先应用 `bootstrap/secret-store`：它建立只读取 `cloudsentinel-secret-source` 中四个指定 Secret 的 Kubernetes Provider `ClusterSecretStore`。明文仍不进入 Git；源 Secret 必须由操作者通过受控终端创建。`cloudsentinel-secrets/` 只创建 Namespace 级 ExternalSecret，由独立 ApplicationSet 自动同步，以保证应用 PreSync Migration 之前所需 Secret 已存在。Secret Application 使用 `ServerSideDiff=true`，避免 External Secrets Webhook 补充的默认字段造成同步成功后仍持续显示 `OutOfSync`。
